#ifndef SERIALPRO_ROBOTWAREHOUSE_H
#define SERIALPRO_ROBOTWAREHOUSE_H
...
namespace warehouse {
    ...
    class WarehouseSerial : public sp::serialPro<head, tail> {
    public:
        enum error {
        ...
        WarehouseSerial() = default;
         WarehouseSerial(const std::string &port, int baud) : sp::serialPro<head, tail>(port, baud) {
        ...
        WarehouseSerial(const WarehouseSerial &other) = delete;

        WarehouseSerial(WarehouseSerial &&other) noexcept: sp::serialPro<head, tail>(std::move(other)) {}
...
#endif //SERIALPRO_ROBOTWAREHOUSE_H
